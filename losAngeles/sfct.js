...
const sfct = async ({
  ...
  let response = body.data.ContainerAvailabiltyByTerm;
  ...
  try {
    if (response[0]) {
      //STATUS
      if (response[0].READYFORDELIVERY.toLowerCase().indexOf("yes") != -1) {
      ...
      //GOODTHRU
      if (response[0].GOODTHRU && response[0].GOODTHRU != "") {
        let dat = moment.tz(response[0].GOODTHRU, timeZone);
        dat.add(
          moment(response[0].GOODTHRU).tz(timeZone).utcOffset * -1,
          "minutes"
        );
      ...
      //FREIGHT
      if (response[0].FREIGHT && response.FREIGHT != "") {
        res.container.freight = response[0].FREIGHT.replace(
          /(<([^>]+)>)/gi,
          ""
        );
      } else {
      ...
      //CUSTOMS
      if (response[0].CUSTOMS && response[0].CUSTOMS != "") {
        res.container.customs = response[0].CUSTOMS.replace(
          /(<([^>]+)>)/gi,
          ""
        );
      } else {
        res.container.customs = null;
      }

      //GROSS WEIGHT
      if (response[0].GROSS_WEIGHT) {
        res.container.extraTracerData.totalWeight =
          response[0].GROSS_WEIGHT.replace(/(<([^>]+)>)/gi, "");
      } else {
      ...
      if (response[0].WEIGHT) {
        res.container.extraTracerData.weight = response[0].WEIGHT;
      } else {
      ...
      if (response[0].CONSIGNEE) {
        res.container.extraTracerData.consigneeName =
          response[0].CONSIGNEE.replace(/(<([^>]+)>)/gi, "");
      } else {
        res.container.extraTracerData.consigneeName = null;
      }

      //CNTR SIZE
      if (response[0]["CNTR SIZE"] && response[0]["CNTR SIZE"] != "") {
      ...
      //VESSEL NAME
      if (response[0]["VESSEL NAME"] && response[0]["VESSEL NAME"] != "") {
        res.container.extraTracerData.vesselName = response[0][
          "VESSEL NAME"
        ].replace(/(<([^>]+)>)/gi, "");
      }

      //VESSEL ETA
      if (response[0]["VESSEL ETA"] && response[0]["VESSEL ETA"] != "") {
        res.container.extraTracerData.eta = response[0]["VESSEL ETA"].replace(
          /(<([^>]+)>)/gi,
          ""
        );
      }
      ...
      if (response[0].VESSELVOYAGE) {
        res.container.extraTracerData.releaseNo =
          response[0].VESSELVOYAGE.replace(/(<([^>]+)>)/gi, "");
      }

      //SSL SCAC
      if (response[0]["SSL SCAC"]) {
        res.container.extraTracerData.scac = response[0]["SSL SCAC"].replace(
          /(<([^>]+)>)/gi,
          ""
        );
      }
    ...
    logger.error(`L4JS FOR POST ${port} and CONTAINER ${ContainerNo} ${e}`);
