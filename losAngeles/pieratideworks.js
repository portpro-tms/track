...
const pieratideworks = async ({
  ...
  try {
    ...
    await page.goto("https://piera.tideworks.com/fc-PA/default.do").catch(err => {
      logger.error(`L4JS FOR ${port} & ${containerNo} ${err}`);
      finalData.caution = true;
      finalData.message = `Could not reach port site currenlty!`;
      browser.close();
      return finalData;
    });;
    ...
    const options = {
      url: `https://piera.tideworks.com/fc-PA/import/default.do?searchBy=CTR&numbers=${containerNo}&6578706f7274=1&method=defaultSearch&d-7957256-e=1&scac=`,
      method: "GET",
      headers: {
    ...
    logger.error(`L4JS FOR ${port} & ${containerNo} ${e}`);
    finalData.container.caution = true;
    finalData.container.message = error.failedScrapeError;
    await browser.close();
    return getResponseValues(finalData.container);
